...
class Ninety:
    ...
    def dailys_api(self, accucode, cityname):
        ...
        ninety_api = f'http://139.159.198.98/pubDataServer/getweatherdailys?apikey={ConfRead.conf_key()}&start={start}&end={end}&citycode={accucode}'
        result = RunMain().run_main('GET', ninety_api)
        # 拿JSON
        data = result.json()
        ...
        # 90日 错误
        e1, e2, e3, e4 = ninety_erro_case(data)
        if e1 or e2 != '':
            erro_city_num += 1
            erro_num = ''
            if e1 != '':
