...
class SendEmail:
    ...
    def alarm(self):
        with open(self.a + "big_alarm.txt", 'a+', encoding="utf-8") as f:
            f.seek(0)
            big_alarm = len(f.readlines())
        if big_alarm >= 1:
            with open(self.a + "big_alarm.txt", 'r+', encoding="utf-8") as f:
                return f.readlines(), big_alarm
        else:
            return ['预警-无错误\n'], 0

    def aqi(self):
        with open(self.a + "big_aqi.txt", 'a+', encoding="utf-8") as f:
            f.seek(0)
            big_aqi = len(f.readlines())
        if big_aqi >= 1:
            with open(self.a + "big_aqi.txt", 'r+', encoding="utf-8") as f:
                return f.readlines(), big_aqi
        else:
            return ['实时空气质量-无错误\n'], 0

    def aqidays(self):
        with open(self.a + "big_aqidays.txt", 'a+', encoding="utf-8") as f:
            f.seek(0)
            big_aqidays = len(f.readlines())
        if big_aqidays >= 1:
            with open(self.a + "big_aqidays.txt", 'r+', encoding="utf-8") as f:
                return f.readlines(), big_aqidays
        else:
            return ['多天空气质量-无错误\n'], 0

    def base(self):
        with open(self.a + "big_base.txt", 'a+', encoding="utf-8") as f:
            f.seek(0)
            big_base = len(f.readlines())
        if big_base >= 1:
            with open(self.a + "big_base.txt", 'r+', encoding="utf-8") as f:
                return f.readlines(), big_base
        else:
            return ['城市信息-无错误\n'], 0

    def condition(self):
        with open(self.a + "big_condition.txt", 'a+', encoding="utf-8") as f:
            f.seek(0)
            big_condition = len(f.readlines())
        if big_condition >= 1:
            with open(self.a + "big_condition.txt", 'r+', encoding="utf-8") as f:
                return f.readlines(), big_condition
        else:
            return ['实况天气-无错误\n'], 0

    def dailys(self):
        with open(self.a + "big_dailys.txt", 'a+', encoding="utf-8") as f:
            f.seek(0)
            big_dailys = len(f.readlines())
        if big_dailys >= 1:
            with open(self.a + "big_dailys.txt", 'r+', encoding="utf-8") as f:
                return f.readlines(), big_dailys
        else:
            return ['多天预报-无错误\n'], 0

    def erro(self):
        with open(self.a + "big_erro.txt", 'a+', encoding="utf-8") as f:
            f.seek(0)
            big_erro = len(f.readlines())
        if big_erro >= 1:
            with open(self.a + "big_erro.txt", 'r+', encoding="utf-8") as f:
                return f.readlines(), big_erro
        else:
            return ['接口错误无返回-无错误\n'], 0

    def hourlys(self):
        with open(self.a + "big_hourlys.txt", 'a+', encoding="utf-8") as f:
            f.seek(0)
            big_hourlys = len(f.readlines())
        if big_hourlys >= 1:
            with open(self.a + "big_hourlys.txt", 'r+', encoding="utf-8") as f:
                return f.readlines(), big_hourlys
        else:
            return ['小时天气-无错误\n'], 0

    def liveInfos(self):
        with open(self.a + "big_liveinfos.txt", 'a+', encoding="utf-8") as f:
            f.seek(0)
            big_liveinfos = len(f.readlines())
        if big_liveinfos >= 1:
            with open(self.a + "big_liveinfos.txt", 'r+', encoding="utf-8") as f:
    ...
    def search(self):
        with open(self.a + "search.txt", mode='a+', encoding='utf-8') as f:
            f.seek(0)
            search = len(f.readlines())
        if search >= 1:
            with open(self.a + "search.txt", mode='r+', encoding='utf-8') as f:
    ...
    def houraqi(self):
        with open(self.a + "big_houraqi.txt", mode='a+', encoding='utf-8') as f:
            f.seek(0)
            houraqi = len(f.readlines())
        if houraqi >= 1:
            with open(self.a + "big_houraqi.txt", mode='r+', encoding='utf-8') as f:
                return f.readlines(), houraqi
        else:
            return ['小时空气质量-无错误\n'], 0

    def erro_city_num(self):
        with open(self.a + "big_erro_citynum.txt", mode='a+', encoding='utf-8') as f:
            f.seek(0)
            erro_num = len(f.readlines())
        return erro_num

    def erro_search_city_num(self):
        with open(self.a + "search_erro_citynum.txt", mode='a+', encoding='utf-8') as f:
            f.seek(0)
            erro_num = len(f.readlines())
        return erro_num

    def erro_live_city_num(self):
        with open(self.a + "live_erro_citynum.txt", mode='a+', encoding='utf-8') as f:
            f.seek(0)
            erro_num = len(f.readlines())
        return erro_num

    def send_email(self, path=text_Path()):
        a1, num1 = self.alarm()
        ...
        a9, num9 = self.liveInfos()
        a9.insert(0, f'【---------- 生活指数-模块错误----------】[{num9}]\n')
        a10, num10 = self.houraqi()
        ...
        # 发送邮件
        yag.send(e_r_name, f'[vivo]-[{self.name}]-[数据]-[国内站点]-[大颗粒-accucode]-[{self.erro_city_num()}]-[生活指数][{self.erro_live_city_num()}]',
                 contents_big)
        # yag.send(['18658886263@163.com'], f'[vivo]-[广州]-[九十日]-[{nine_num}]', contents_ninety)
        yag.send(e_r_name, f'[vivo]-[{self.name}]-[数据]-[国内站点]-[搜索]-[{self.erro_search_city_num()}]', contents_search)
        re_big_city_erro_num(path)
