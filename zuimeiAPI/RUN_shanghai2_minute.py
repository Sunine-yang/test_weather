...
def run_minute_big():
    ...
    send_time = 5
    while True:
        big_r = big.min_code_to_name('60970', '广元市')
        live_r = live.min_liveInfos_api('59565', '高邮市')
        if nine_time == 1:
...
def run_minute_geo():
    ...
    send_time = 5
    while True:
        geo_r = geo.min_geo_to_name('121.447935', '37.463822', '烟台市')
        if nine_time == 1:
...
def run_minute_nine():
    ...
    send_time = 5
    while True:
        nin_r = nin.min_dailys_api('61488', '玉溪市')
        if nine_time == 1:
