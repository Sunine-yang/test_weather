...
class Large_Particles:
    ...
    def large_particles_check(self,unm1,num2):
        global data_len, city, dailys, condition, hourlys, liveInfos, city_check, condition_check, hourlys_check, dailys_check, mobilelink
        Url_data().wait_big_sql_data('large_particles','sql',unm1,num2)
        read_sql=Url_data().read_sql_data(str(num2))
        ...
        for i in range(len(read_sql)):
            ...
            if result=='':
                ...
                self.check.list_data.append(result)
    # 城市信息检验
        ...
        for l in range(len(get_url["data"]["dailys"])):
            ...
            if Data_analysis.hour_time_handle()>9 and int(Data_analysis.days_time_handle())>1:
                data=self.check.comparison_check(int(Data_analysis.days_time_handle(get_url["data"]["dailys"]["dailyweathers"][0]["publictime"])),
                                            int(Data_analysis.days_time_handle())-1, '| 多天预报时间:(%s/%s)')
                if data=='':
                    pass
                else:
                ...
                data=self.check.comparison_check(int(Data_analysis.days_time_handle(get_url["data"]["dailys"]["dailyweathers"][0]["publictime"])),
                    int(Data_analysis.month_days_check()), '| 多天预报时间:(%s/%s)')
                if data == '':
                    pass
                else:
                    self.check.list_data.append(sql_data + data)
    ...
    def hourlys_check(self, get_url,sql_data):
        for h in range(len(get_url["data"]["hourlys"])):
            ...
            data=self.check.comparison_check(int(Data_analysis.hour_time_handle(get_url["data"]["hourlys"]["hourlyweathers"][0]["date"]))
                                        ,Data_analysis.hour_time_handle()-1, '| 小时天气:(%s/%s)')
            if data == '':
                pass
            else:
                self.check.list_data.append(sql_data+data)
            for u in range(len(get_url["data"]["hourlys"]["hourlyweathers"])):
                ...
                if result=='':
                    ...
                    data='| 小时天气 %s'%Data_analysis.time_transform(get_url["data"]["hourlys"]["hourlyweathers"][u]["date"])+\
                        result
                    return data
...
if __name__ == '__main__':
    Large_Particles().large_particles_start(0,10000,'全国')