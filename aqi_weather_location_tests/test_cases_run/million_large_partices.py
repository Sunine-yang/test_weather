...
class Large_Particles:
    ...
    def data_check(self,get_url):
        ...
        for a in range(len(get_url["data"])):
            try:
                ...
                condition=self.check.comparison_none_check(get_url,'| condition:(%s)')
                dailys=self.check.comparison_none_check(get_url,'| dailys:(%s)')
                hourlys=self.check.comparison_none_check(get_url,'| hourlys:(%s)')
    ...
    def dailys_check(self, get_url,sql_data):
        for l in range(len(get_url)):
            ...
            for m in range(len(get_url["dailyweathers"])):
                self.dailyweathers_check(get_url["dailyweathers"][m]["conditionNight"],'conditionNight')
                self.dailyweathers_check(get_url["dailyweathers"][m]["conditionDay"],'conditionDay')
    ...
    def hourlys_check(self, get_url,sql_data):
        datas = self.check.time_check(
            (int(Data_analysis.hour_time_handle(get_url[0]["date"])))
            , Data_analysis.hour_time_handle() , '| time:(%s/%s)')
        for h in range(len(get_url)):
            self.check.comparison_check(len(get_url), 2, '| 字节长度:(%s/%s)')
            self.check.comparison_none_check(get_url["expiretime"])
            self.check.comparison_none_check(get_url)
            self.hourlyweathers_check(get_url["hourlyweathers"])
    def hourlyweathers_check(self,get_url):
        for u in range(len(get_url)):
            ...
            if result=='':
                ...
                data_report='| 小时天气 %s'%Data_analysis.time_transform(get_url[u]["date"])+\
                    result
                return data_report
